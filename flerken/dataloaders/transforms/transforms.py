...
class ToPILImage(BaseTransformation):
...
class Lambda(BaseTransformation):
...
class FiveCrop(BaseTransformation):
...
class TenCrop(BaseTransformation):
...
class RandomRotation(BaseTransformation):
...
class RandomAffine(BaseTransformation):
    ...
    def __call__(self, img):
        ...
        return F.affine(img, *self.ret, resample=self.resample, fillcolor=self.fillcolor)
...
class Grayscale(BaseTransformation):
...
class RandomGrayscale(BaseTransformation):
